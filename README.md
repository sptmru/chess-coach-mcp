...
- **`untrusted_origin` after submitting the sign-in form:** the authorization page must use `Referrer-Policy: same-origin`. Overriding it with `no-referrer` makes browsers submit the form with `Origin: null`, which the server rejects. Rebuild the app after updating, start a fresh OAuth flow, and ensure your proxy preserves this header and that the browser URL matches `PUBLIC_URL`.
- **Browser blocks the OAuth callback with `form-action`:** the authorization page permits form submission to itself and the validated client's callback origin. A proxy must not replace this CSP with `form-action 'self'`, which also blocks the cross-origin redirect after consent.
